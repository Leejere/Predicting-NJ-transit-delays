...
# The above code chunks takes too long to run. Here, we import a data-set that has been prepared in advance
...
# The above code chunk takes too long to run. Here, we import a data-set that has been prepared in advance
...
In Figure 23. a New Jersey Transit operator could easily view whether a train is predicted to delay and how long it is predicted to delay by filtering prediction model, time of prediction, scheduled time, and so on. They can also view real-time average delays by line, by station, etc.
...
In Figure 24., once the operator identifies the delayed trains, measures can be taken to mitigate the situation, e.g., by adjusting train speeds or directing more staff to accelerate passenger flows at the delayed stations. The operator can first model the potential effects of these measures before making the decision. Our algorithm could also be used for **retrospective evaluation**, i.e. to reveal the primary factors leading to systematic train delays, based on which to improve the entire transit network and to  provide a better service for passengers. 

**For Video Presentation, please click [here](https://www.youtube.com/watch?v=QlJJaxhCYwQ)**
