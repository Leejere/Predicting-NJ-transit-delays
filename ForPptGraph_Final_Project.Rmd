...
```{r read_trains_data}
trainsSep = read.csv('./MUSA508_Final/2019_09.csv') %>%
...
trainsOct = read.csv('./MUSA508_Final/2019_10.csv') %>%
  filter(., type == 'NJ Transit' & status == 'departed' &
           line != 'Princeton Shuttle') %>%
  mutate(date = ymd(date),
         scheduled_time = ymd_hms(scheduled_time),
...
```{r,fig.width=36, fig.height=24,out.width = '75%',fig.align = 'center'}
#不同线路的平均延迟时间
alllines <- trainsWithGeometry%>%
  st_drop_geometry()%>%
...
list.files(pattern = '*.png', full.names = TRUE) %>% 
        ...
        image_write("allLines.gif") # write to current dir
```


```{r,fig.width=12, fig.height=8,out.width = '75%',fig.align = 'center'}
for (i in linelist){
  save <- trainsWithGeometry%>%
  dplyr::select(to,delay_minutes,line)%>%
  na.omit()%>%
  group_by(to,line)%>%
  summarise(averageDelay=mean(delay_minutes))%>%
  filter(line == i)%>%
  ggplot()+
  geom_sf(data=st_union(njCounties),color="black",size=1.5,fill = "transparent")+
  ...
  labs(title = paste(i,"Delay Distribution",sep = ""))+
  mapTheme(10,25)+
  theme(legend.position = "bottom")
  ggsave(paste(i,"geo.png",sep = ""))}
...
st_distance(trainIDAndStations[1,], firstStations[1,])
...
ggplot(trainsTest) +
  geom_point(aes(x = delay, y = predict2hSqSqWTileWDist)) +
  geom_abline(slope = 1, intercept = 0)
