...
ggmap(base_map) + 
  ...
  labs(title = "Map of NJ Transit Railway Lines", 
       subtitle = "",
       x="",y="")+
  mapTheme()
...
kable(trainIDforWholeTrip,align = 'c',caption = '<center>Table 1. trainID for a whole trip</center>')%>%
...
kable(difTrainIDs,align = 'c',caption = '<center>Table 2. Different TrainIDs within the same day & Express or normal lines</center>')%>%
  kable_classic(full_width = T)%>%
  kable_styling(position = "center")%>%
  column_spec(2, bold = T,color = palette1_assist)%>%
...
kable(TrainIDforDifDay,align = 'c',caption = '<center>Table 3. TrainID is allocated for the specific time slot everyday</center>')%>%
  kable_classic(full_width = T)%>%
  kable_styling(position = "center")%>%
...
```{r Average delay time,message = FALSE, warning = FALSE, results='hide',fig.height = 4,out.width = '75%',fig.align = 'center'}
...
```{r ReadGif for alltemporal, echo=FALSE,fig.height = 6,out.width = '100%',fig.align = 'center',fig.cap="Average train delays of each line by the progression of time"}
...
ggplot(lagVarScatter %>% 
         mutate(Variable = recode(Variable,
                ...
                Variable = factor(., levels = c("Line delay lag",
                                                "Station delay lag: this station",
                                                "Station delay lag: previous station",
                                                "Station delay lag:\nprevious of previous station",
                                                "Same-train previous delay"))) %>% 
         sample_n(30000), aes(Value, delay)) +
...
*How good is the Baseline model?* Figure 10. shows by scatter plot the relationship between the *predicted* delays and the *actual delays*, and Figure 11. plots the the differences between the predicted and actual delays for groups of short, median or long delays. It is evident that this baseline model under-predicts the long delays. This is probably because the distribution of delays is highly skewed, as shown in the histogram in Figure 12.

```{r delay distribution hist, message=FALSE, warning=FALSE,fig.height=4,out.width='50%',fig.align='center', fig.cap = "Figure 12. Distribution of delays"}
ggplot(trainsTrain) +
...
#### 4.2 Improving Baseline

To avoid under-predicting long delays, we make the following adjustment to the Baseline model. First, we add a peak station-hour effect, as explained in Section 2.4. Then, we try to apply a logarithmic or square-root transformation to the dependent variable and/or predictors. However, as shown in Figure 13., the Logarithmic Transformation Model now drastically over-predicts long delays. On the other hand, the Square-Root Transformation Model does much better in predicting those long delays. Furthermore, we devise an addition model which applies a square-root transformation not only to the dependent variable, but also to the time-lag features (predictors).

Figure 14. exhibits the relationship between *actual delay* and *absolute prediction error* of the above-mentioned four models: 1) Baseline, 2) Logarithmic Transformation, 3) Square-Root Transformation to Dependent Variable, and 4) Square-Root Transformation to Dependent and Independent Variables. Table 4. summarizes the MAE (mean absolute error) corresponding to these four models. By applying square-root transformations to predictors, the fourth model does not perform better than the third model, which applys a square-root transformation only to the dependent variable.
...
kable(ErrComparison,align = 'c',caption = '<center>Table 4. MAEs for the four linear models </center>',table.attr = "style='width:75%;'")%>%
  kable_classic(full_width = T)%>%
  kable_styling(position = "center")%>%
...
#### 4.4 Logit: better for mid-range delays
...
```{r logit results plot, message=FALSE, fig.cap = 'Figure 15. Distribution of predicted delay possibilities by observed outcome', warning=FALSE,fig.height=6, fig.width = 8,fig.align='center'}
...
```{r roc, message=FALSE, warning=FALSE,fig.height=4,out.width='50%',fig.align='center'}
ggplot(lateProbs, aes(d = as.numeric(lateProbs$outcome), m = probs)) +
  geom_roc(n.cuts = 50, labels = FALSE, colour = color2) +
  style_roc(theme = theme_grey) +
  geom_abline(slope = 1, intercept = 0, size = 1.5, color = 'grey') +
  labs(title = "ROC Curve - delayed or not delayed") +
  plotTheme()
```
...
```{r logit eval, message=FALSE, warning=FALSE,fig.height=4,out.width='50%',fig.align='center'}
...
cbind(
  ...
  gather(Metric, Value) %>%
  kable(caption = '<center><small><small><small>Table 6. Model results of the logistic model</small></small></small></center>') %>% 
...
We cross-validate (CV) both the final linear model and the logic model by day; that is, each day takes turns to be the test set on which a model trained on the rest of the days is tested.
Figure 16. shows the CV results of the linear model, and Figure 17. shows the CV results of the logistic model. Both suggest that the models predict the most errors during the weekend, despite that the day of week is already controlled for.
...
#### 5.2 Generalizability by station and time

Figure 18. Shows the MAE of the linear model for each station during the two weeks in the test set. We also calculate the MAE of each station by day of week (Figure 19.) and hour of day (Figure 20.) The results suggest that the generalizability of the linear model is generally acceptable, but that it generalizes better for weekdays and during daytime.


```{r MAE by station by week, message=FALSE, warning=FALSE,fig.height = 12,out.width='75%',fig.align='center'}
...
byStationByWeek %>%
  ...
  geom_sf(data = njCounties,
          color= 'black', size=1,linetype = 'dashed', fill = NA) +
...
```{r MAE by station by day of week, eval=FALSE, message=FALSE, warning=FALSE,fig.height = 12,out.width='75%',fig.align='center'}
...
anim_save("byStationByDayOfWeekAnimationPresent.gif", byStationByDayOfWeekAnimation,
          duration=10, renderer = gifski_renderer() ,height = 1500, width =1000)
```
...
```{r MAE by hour, eval=FALSE, message=FALSE, warning=FALSE,fig.height = 12,out.width='75%',fig.align='center'}
byStationByHour = 
  finalComparison %>%
...
anim_save("byStationByHourAnimationPresent.gif", byStationByHourAnimation,
          duration=10, renderer = gifski_renderer(), height = 1500, width = 1000)
```
...
```{r MAE by line2,message=FALSE, warning=FALSE,fig.height = 12,out.width = '75%',fig.align = 'center'}
byLine %>%
  ggplot() +
  geom_sf(data = st_union(njCounties),
          color='black', size=2, fill = NA)+
...
#### 5.3 Generalizability by short and long delays
...
### 6 An app for New Jersey Transit

Now, we demonstrate how an app could be created based on the above models, that will be of assistance to the New Jersey Transit.
...
A New Jersey Transit operator could easily view whether a train is predicted to delay and how long it is predicted to day  by filtering time of prediction (Figure 23.), scheduled time (Figure 24.), and so on. They can also view real-time average delays by line (Figure 25.), by station (Figure 26.), etc.

Once the operator identifies the delayed trains, measures can be taken to mitigate the situation, e.g., by adjusting train speeds or directing more staff to accelerate passenger flows at the delayed stations (Figure 26.). The operator can first model the potential effects of these measures before making the decision (Figure 27.)

<br>
<br>

```{r ReadGif for NJ transit, echo=FALSE,fig.height = 6,out.width = '75%',fig.align = 'center'}
knitr::include_graphics("NJ-transitPart1.gif")
```

<br>
<br>
```{r ReadGif for NJ transit2, echo=FALSE,fig.height = 6,out.width = '75%',fig.align = 'center'}
knitr::include_graphics("NJ-transitPart2.gif")
```

<br>
<br>
```{r ReadGif for NJ transit3, echo=FALSE,fig.height = 6,out.width = '75%',fig.align = 'center'}
knitr::include_graphics("NJ-transitPart3.gif")
```
